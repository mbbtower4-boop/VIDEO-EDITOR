...
async function generate(manifest, prompt, jobId) {
  ...
  const args = ops.buildLlamaArgs(manifest.models.llm, promptFile);
  try {
    let r = await tools.run(manifest.llamaExe, args, { jobId });
    if (r.killed) throw new Error('cancelled');
    // portable package on a PC with a different GPU — retry with the CPU build
    if (r.code !== 0 && manifest.llamaCpuExe && manifest.llamaCpuExe !== manifest.llamaExe) {
      r = await tools.run(manifest.llamaCpuExe, args, { jobId });
      if (r.killed) throw new Error('cancelled');
    }
