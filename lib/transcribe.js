...
async function transcribe(manifest, video, opts, onProgress, jobId) {
  ...
  const model = o.translate
    ...
    : (manifest.models && (manifest.models.default || manifest.models.turbo));
  if (!model) {
  ...
  try {
    let r = await tools.run(manifest.whisperExe, args, runOpts);
    if (r.killed) throw new Error('cancelled');
    // The portable package may run on a PC whose GPU differs from the machine
    // that wrote the manifest — retry once with the bundled CPU build.
    if (r.code !== 0 && manifest.whisperCpuExe && manifest.whisperCpuExe !== manifest.whisperExe) {
      r = await tools.run(manifest.whisperCpuExe, args, runOpts);
      if (r.killed) throw new Error('cancelled');
    }
