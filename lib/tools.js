...
// The portable package ships a manifest with RELATIVE paths (relative to the
// tools directory) so it works wherever the folder is unzipped; a manifest
// written by scripts/setup.js on this machine has absolute paths.
function resolveToolPath(p) {
  if (!p || typeof p !== 'string') return p;
  return path.isAbsolute(p) ? p : path.join(path.dirname(MANIFEST_PATH), p);
}
...
function loadManifest() {
  try {
    ...
    for (const k of ['ffmpeg', 'ffprobe', 'whisperExe', 'whisperCpuExe', 'llamaExe', 'llamaCpuExe']) {
      m[k] = resolveToolPath(m[k]);
    }
    if (m.models) {
      for (const k of Object.keys(m.models)) m.models[k] = resolveToolPath(m.models[k]);
    }
